...
enum
{
            MODE_LIST,                      // Simply list all sensors with values and recommendations
            MODE_COMPARE,                   // List only sensors with overvalues (and recommendations)
};
...
uint8_t     buildingDisplayState = DISPL_LOCATIONS; // building display state
...
namespace   cfg
{
  ...
  bool correctHumi = CORRECTHUMI;                           // true = correct PM values for humidity
  bool intelliHumi = INTELLIHUMI;                           // true = intelligent humidity calculation
  ...
  //
  char nameRef[LEN_LOCATION_NAME]      = NAME_REF;
	char pmDryRef[LEN_SENSOR_API_1]      = PM_DRYREF;
	char humiDryRef[LEN_SENSOR_API_2]    = HUMI_DRYREF;
  char pmWetRef[LEN_SENSOR_API_1]      = PM_WETREF;
	char humiWetRef[LEN_SENSOR_API_2]    = HUMI_WETREF;
  //
...
// Check value min/max
uint checkValue(uint valnow, const uint minval, const uint defval, const uint maxval) 
{
...
// Transform a value to a rounded string, or "N/A" string if applicable
String rounder(float value, uint8_t precision) 
{
  ...
  // Format the float with the given precision using sprintf
...
// Check settings, & if changed, return true
...
static void formatForDisplay()
{
  ...
  // Switch fixed to scroll message if message length longer than 24 chars
  if ((strlen(forDisplay) > 24) && (formatChar == FIXED_TEXT)) 
  {
  ...
  if ((strlen(forDisplay) <= 23) && (formatChar == SCROLL_TEXT))  
  {
